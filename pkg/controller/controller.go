...
func main() {
	...
	var command = cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			subscriptions.SetAnnotationPrefix("notifications.kluctl.io")
			...
			notificationsFactory := api.NewFactory(api.Settings{
				ConfigMapName: "kluctl-notifications-cm",
				SecretName:    "kluctl-notifications-secret",
				InitGetVars: func(cfg *api.Config, configMap *v1.ConfigMap, secret *v1.Secret) (api.GetVars, error) {
					return func(obj map[string]interface{}, dest services.Destination) map[string]interface{} {
						return map[string]interface{}{"kluctldeployment": obj}
					}, nil
				},
			}, namespace, secrets, configMaps)

			// Create notifications controller that handles Kubernetes resources processing
			kluClient := dynamic.NewForConfigOrDie(restConfig).Resource(schema.GroupVersionResource{
				Group: "gitops.kluctl.io", Version: "v1beta1", Resource: "kluctldeployments",
			})
			kluInformer := cache.NewSharedIndexInformer(&cache.ListWatch{
				ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
					return kluClient.List(context.Background(), options)
				},
				WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
					return kluClient.Watch(context.Background(), metav1.ListOptions{})
				},
			...
			go kluInformer.Run(context.Background().Done())
			if !cache.WaitForCacheSync(context.Background().Done(), secrets.HasSynced, configMaps.HasSynced, kluInformer.HasSynced) {
